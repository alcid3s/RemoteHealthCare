...
namespace RemoteHealthCare.Bikes
{
    ...
    internal class RealBike : IBike
    {
        ...
        public async void SetResistance(double resistance)
        {
            byte resistanceByte = (byte)Math.Round(resistance * 200);
            Console.WriteLine("trying to write resistance " + resistance + " as " + resistanceByte);
            byte[] values = { 0xa4, 0x09, 0x4e, 0x05, 0x30, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, resistanceByte, (byte)(0xe6 ^ resistanceByte) };
            while (_bike == null);
